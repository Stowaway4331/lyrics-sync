...
- [x] 7.21 Chats are stored on the device as threads: "New chat" button and a list of past chats
- [ ] 7.22 Delete icon on a chat archives it: moved from the device to server storage
...
- 2026-09-25, item 7.21: chat threads are stored on the device (a JSON file in the app's documents folder via `expo-file-system`; `localStorage` on web), saved after each question and after each reply. The chat header has "Chat history" (list of threads, newest first, with "New chat" on top) and "New chat". The Worker no longer stores chat messages: the app sends the last 20 messages of the thread with each question (each capped at 4,000 characters). On first launch the chat history the server kept before is imported once into a thread. Production check: a follow-up "What year was it released?" with a Bohemian Rhapsody exchange as history was answered "1975", and the server's chat store stayed empty. Needs a check on the phone.
