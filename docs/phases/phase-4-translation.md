...
- [x] 4.4 Share one translation job per song and language (KV caches misses for ~60 s)
...
- 2026-09-24, item 4.4: each (song, language) now has one fixed Workflow instance id (`tr-<lrclibId>-<lang>`). Production check with "Alors On Danse" → German: two devices requesting at once shared the job `tr-37123784-german`; the second device got the result by polling `/jobs/:id` (read from the instance output); a third request right after completion returned the lines immediately, inside the KV window.
