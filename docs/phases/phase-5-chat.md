...
- [ ] 5.4 Prefer LRCLIB results whose title matches the candidate when verifying
...
- 2026-09-24, item 5.3 baseline (`node worker/eval/run.mjs` against `wrangler dev`, 20 cases in `worker/eval/lyric-search.json`): **top-1 18/20 (90%)**, top-3 18/20, expected song labelled "match" 16/20.
    ...
    - Real miss: "Something Just Like This". LRCLIB's fuzzy search returned "Into The Storm" for the right candidate, and verification accepted it without checking the title.
